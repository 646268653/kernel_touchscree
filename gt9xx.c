...
void gtp_int_sync(s32 ms)
{
   ...
   w55fa93_gpio_set_output(GPIO_GROUP_A, 0);
   w55fa93_gpio_set(GPIO_GROUP_A, 0, 0);
    msleep(ms);
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
   ...
   w55fa93_gpio_set(GPIO_GROUP_A, 0, client->addr == 0x14);

    msleep(2);                          // T3: > 100us
    //GTP_GPIO_OUTPUT(gtp_rst_gpio, 1);
    w55fa93_gpio_set(GPIO_GROUP_A, 1, 1);
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    ret = w55fa93_gpio_configure(GPIO_GROUP_A, 0);
    if (ret < 0) 
    {
	...
	//writel((1 << 0x2), REG_AIC_SCCR);
        //ts->client->irq = gpio_to_irq(gtp_int_gpio);
	GTP_INFO("===GPIOA REG_GPIO_OMD = 0x%x=======\n", readl(REG_GPIOA_OMD));
	GTP_INFO("===GPIOA REG_GPIO_PUEN= 0x%x=======\n", readl(REG_GPIOA_PUEN));
	GTP_INFO("===GPIOA REG_GPIO_DOUT= 0x%x=======\n", readl(REG_GPIOA_DOUT));
	GTP_INFO("===GPIOA REG_GPIO_PIN = 0x%x=======\n", readl(REG_GPIOA_PIN));
	GTP_INFO("===GPIOA REG_IRQSRCGPA = 0x%x=======\n", readl(REG_IRQSRCGPA));
	GTP_INFO("===GPIOA REG_IRQENGPA = 0x%x=======\n", readl(REG_IRQENGPA));
	GTP_INFO("===GPIOA REG_AIC_SCR1 = 0x%x=======\n", readl(REG_AIC_SCR1));
	
    }

    //ret = GTP_GPIO_REQUEST(gtp_rst_gpio, "GTP RST PORT");

    ret = w55fa93_gpio_configure(GPIO_GROUP_A, 1);
    if (ret < 0) 
    {
   ...
   // GTP_GPIO_AS_INPUT(gtp_rst_gpio);
    w55fa93_gpio_set(GPIO_GROUP_A, 1, 0);
