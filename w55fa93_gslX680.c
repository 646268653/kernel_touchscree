...
//#include "samsung_gslX680.h"
#include <mach/w55fa93_gslX680.h>
//#define GSL_DEBUG
//#define GSL_MONITOR
...
{
	printk("%s\n",__FUNCTION__);
	w55fa93_gpio_configure(GPIO_GROUP_E,0);	   //GPE0
	w55fa93_gpio_set_output(GPIO_GROUP_E,0);

	w55fa93_gpio_set(GPIO_GROUP_E,0,0);	 // low level
	msleep(3);

	w55fa93_gpio_configure(GPIO_GROUP_A,0);
	w55fa93_gpio_set_input(GPIO_GROUP_A,0);
	writel(readl(REG_GPIOA_PUEN) & ~(BIT0), REG_GPIOA_PUEN);

	writel(readl(REG_IRQSRCGPA) & ~(0x3), REG_IRQSRCGPA);
	writel((readl(REG_IRQENGPA) | (0x1 << 16)) & ~(0x1), REG_IRQENGPA);
	writel(readl(REG_AIC_SCR1) | 0x00C70000, REG_AIC_SCR1);
	
	writel(0x00000001, REG_IRQTGSRC0);
	writel((1 << IRQ_PORT), REG_AIC_SCCR);	
...
//	gpio_direction_output(GPIO_TS_WAKE, 0);
	w55fa93_gpio_set(GPIO_GROUP_E,0,0);	//GPE0 low level
	return 0;
}
...
static int gslX680_shutdown_high(void)
{
	...
	w55fa93_gpio_set(GPIO_GROUP_E,0,1);
	return 0;
}
...
{	
	...
	writel(src & (0x00000001), REG_IRQTGSRC0);
...
static int gslX680_ts_init(struct i2c_client *client, struct gsl_ts *ts)
{
	...
	set_bit(ABS_X, input_device->absbit);   //ABS_X ABS_Y 
	set_bit(ABS_Y, input_device->absbit);
	set_bit(ABS_PRESSURE, input_device->absbit);
//	set_bit(ABS_MT_POSITION_X, input_device->absbit);   //ABS_MT_POSITION_X ABS_MT_POSITION_Y
...
//	__set_bit(INPUT_PROP_DIRECT, input_device->propbit);    // comment by jjw in 2017.3.10
...
//	input_set_abs_params(input_device,ABS_MT_POSITION_Y, 0, SCREEN_MAX_Y, 0, 0);
	...
	input_set_abs_params(input_device,ABS_PRESSURE, 0, 1000, 0, 0);
