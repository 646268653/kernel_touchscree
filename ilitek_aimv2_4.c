...
struct i2c_data {
	...
	// work queue for interrupt use only to mount the work queue func
...

{
	...
	w55fa93_gpio_set(GPIO_GROUP_E,0,1);	 // low level
	...
	writel(readl(REG_IRQSRCGPA) & ~(0x03), REG_IRQSRCGPA);
	//writel(readl(REG_IRQSRCGPA) | (0x01 << 1), REG_IRQSRCGPA);
	writel((readl(REG_IRQENGPA) | (0x1 << 0)) & ~(0x1 << 16), REG_IRQENGPA);

	writel(readl(REG_AIC_SCR1) | 0x00870000, REG_AIC_SCR1);
...
static int ilitek_i2c_process_and_report(void)
{
	...
	unsigned char tp_id,max_point=1;
		...
		for(i = 0; i < max_point; i++){
			...
			if(tp_status){
				...
				if(touch_key_hold_press == 0){
					...
					input_event(i2c.input_dev, EV_ABS, ABS_MT_POSITION_X, (x*1024)/i2c.max_x);
					input_event(i2c.input_dev, EV_ABS, ABS_MT_POSITION_Y, (y*600)/i2c.max_y);
...
/*
description
rq	work queue function for irq use
parameter
	work
		work queue
return
	...
	int irq, void *dev_id)
{
	...
	writel(readl(REG_IRQTGSRC0) & (0x00000001), REG_IRQTGSRC0);
	#ifndef CLOCK_INTERRUPT
	printk(KERN_ALERT "====================i2c.irq_status entry!!!!==============");
		if(i2c.irq_status == 1 ){
