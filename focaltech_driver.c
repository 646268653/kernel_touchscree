...
static const struct i2c_device_id ft5206_tpd_id[] = {{TPD_DEVICE, 0}, {}};
#ifdef ADD_I2C_DEVICE_ANDROID_4_0
static struct i2c_board_info __initdata ft5206_i2c_tpd = { I2C_BOARD_INFO(TPD_DEVICE, (0x70 >> 1))};
#else
unsigned short force[] = {0,(0x70 << 1),I2C_CLIENT_END,I2C_CLIENT_END};
...
static void tpd_charger_check(int resume)
{
    ...
    FTS_DBG("********************[FTS] Enter tpd_charger_check********************\n");

    if (resume || (ft5x46_charger_state != charger_state))
    {
        ft5x46_charger_state = charger_state;
    FTS_DBG("********************[FTS] ft5x46_charger_state != charger_state********************\n");
...
//add TP vendor  linan
    err = fts_read_reg(i2c_client, FT_REG_FW_VER, &fw_version);

    if (err < 0)
    {
        pr_err("arad FT_REG_VENDOR_ID failed, ret: %d value: %d\n",err, fw_version);
    }
    printk("FT_REG_VENDOR_ID is %d\n",fw_version);
    tpd_load_status = 1;
